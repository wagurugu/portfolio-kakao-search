/******************** global init *******************/
var auth = "KakaoAK ff6364760377ba9101a326380dae96ab";
...
function setImageLists(r) {
  ...
  // Observer 처리 - category 세부분류에 따라 pager 연결
  $(".lists").after(
    '<div class="observer d-flex justify-content-center py-5"><i class="fa fa-spin fa-spinner fa-2x"></i></div>'
  );
...
function setBookLists(r) {
  ...
  r.forEach(function (v, i) {
    ...
    var thumbnail =
      v.thumbnail !== "" ? v.thumbnail : "http://via.placeholder.com/120x174/eee?text=No+image";
...
function setPager(totalRecord) {
  ...
  for (var i = startPage; i <= endPage; i++) {
    if (i === page)
      $('<button class="bt-page active" data-page="' + i + '">' + i + "</button>")
        .insertBefore(".pager-wrap .bt-next")
        .click(onPagerClick);
    else
      $('<button class="bt-page" data-page="' + i + '">' + i + "</button>")
        .insertBefore(".pager-wrap .bt-next")
        .click(onPagerClick);
  }
  if (page === 1) $(".pager-wrap .bt-first").attr("disabled", true);
  else $(".pager-wrap .bt-first").attr("disabled", false);
  if (startPage === 1) $(".pager-wrap .bt-pager-prev").attr("disabled", true);
  else $(".pager-wrap .bt-pager-prev").attr("disabled", false)[0].dataset["page"] = startPage - 1;
  if (page === 1) $(".pager-wrap .bt-prev").attr("disabled", true);
  else $(".pager-wrap .bt-prev").attr("disabled", false)[0].dataset["page"] = page - 1;
  if (page === totalPage) $(".pager-wrap .bt-next").attr("disabled", true);
  else $(".pager-wrap .bt-next").attr("disabled", false)[0].dataset["page"] = page + 1;
  if (endPage === totalPage) $(".pager-wrap .bt-pager-next").attr("disabled", true);
  else $(".pager-wrap .bt-pager-next").attr("disabled", false)[0].dataset["page"] = endPage + 1;
  if (page === totalPage) $(".pager-wrap .bt-last").attr("disabled", true);
  else $(".pager-wrap .bt-last").attr("disabled", false)[0].dataset["page"] = totalPage;
}
